...
def install_pkg(pkg_spec, install_path):
    data = get_pkg_metadata(pkg_spec)
    latest_ver = data["info"]["version"]
    tgz_packages = [x for x in data["releases"][latest_ver] if ".tar.gz" in x["filename"]]
    whl_packages = [x for x in data["releases"][latest_ver] if ".whl" in x["filename"]]
    packages = whl_packages if len(whl_packages) > 0 else tgz_packages
    upy_platforms = [x for x in data["info"]["classifiers"] if "MicroPython" in x]
    
    assert len(upy_platforms) > 0 and len(packages) == 1, "This doesn't look like MicroPython package"
    ...
    if len(whl_packages) > 0:
        with ZipFile(f1.makefile()) as z:
            install_zip(z) # Do magic with z.infolist() and z.open()
    else:
        s = read_lines(f1)
        try:
